...
# Fungsi index
# -----------
# 🖼️ Merender halaman utama (index.html) untuk aplikasi.
def index(request):
...
# Fungsi feature1
# ---------------
# ⚙️ Menangani logika untuk Fitur 1, termasuk pemrosesan data POST (baik dari form biasa maupun AJAX).
# Fitur ini menerima deskripsi dan persentase input dari pengguna,
# kemudian memanggil fungsi `get_combined_recommendations` untuk mendapatkan rekomendasi.
# Hasilnya bisa berupa halaman HTML yang dirender kembali atau respons JSON jika permintaan AJAX.
...
# Fungsi process_url
# ------------------
# 🔗 Memproses URL yang dikirim melalui permintaan POST (AJAX).
# Fungsi ini memanggil fungsi `insight` untuk mengambil data parfum berdasarkan URL,
# kemudian mengembalikan data numerik dan data input parfum dalam format JSON.
...
# Fungsi insight
# --------------
# 💡 Mengambil informasi detail parfum dari DataFrame utama berdasarkan URL yang diberikan.
# Fungsi ini mengembalikan dua DataFrame: satu berisi semua data input untuk parfum tersebut,
# dan satu lagi berisi subset data numerik yang relevan (tidak termasuk kolom rating dan kolom kosong/nol).
def insight(url):
...
# Fungsi feature2
# ---------------
# 🔍 Menangani logika untuk Fitur 2, yang melibatkan pencarian dan paginasi data parfum.
# Fungsi ini menerima query pencarian (opsional) dan nomor halaman.
# Jika ada query, fungsi akan melakukan pencarian fuzzy pada kolom 'Perfume_Name', 'Brand', dan 'Description'.
# Hasil pencarian (atau semua data jika tidak ada query) kemudian dipaginasi dan dikirimkan ke template `feature2.html`.
# Fungsi ini juga mengirimkan daftar nama notes dalam format JSON untuk digunakan oleh JavaScript di sisi klien.
